...
if ( ! class_exists( 'BP_Zoom_Ajax' ) ) {
	...
	class BP_Zoom_Ajax {
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_nopriv_zoom_meeting_get_single_meeting', array( $this, 'zoom_meeting_get_single_meeting' ) );
		}
		...
		public function zoom_meeting_add() {
			...
			if ( ! empty( $zoom_meeting['code'] ) && in_array( $zoom_meeting['code'], array( 201, 204 ), true ) ) {
					...
					if ( ! empty( $zoom_meeting['response']->occurrences ) ) {
						...
						foreach ( $zoom_meeting['response']->occurrences as $meeting_occurrence ) {
							...
							if ( false === $occurrence_id ) {
								...
								if ( $occurrence_date->format( 'U' ) > wp_date( 'U', strtotime( 'now' ), new DateTimeZone( 'UTC' ) ) ) {
						...
						if ( $is_update ) {
							$occurrences = bp_zoom_meeting_get( array( 'parent' => $meeting_id ) );

							if ( ! empty( $occurrences['meetings'] ) ) {
								$occurrence_ids     = wp_parse_id_list( wp_list_pluck( $occurrences['meetings'], 'meeting_id' ) );
								$api_occurrence_ids = wp_parse_id_list( wp_list_pluck( $zoom_meeting['response']->occurrences, 'occurrence_id' ) );
				...
				if ( ! empty( $zoom_meeting['response'] ) ) {
					bp_zoom_meeting_update_meta( $id, 'zoom_details', wp_json_encode( $zoom_meeting['response'] ) );
			...
			if ( ! empty( $meetings ) ) {
				foreach ( $meetings as $meeting_id ) {
					...
					if ( 200 === $meeting_info['code'] && ! empty( $meeting_info['response'] ) ) {
						...
						$object = json_decode( wp_json_encode( $meeting_info['response'] ), true );
						...
						bp_zoom_meeting_update_meta( $meeting->id, 'zoom_details', wp_json_encode( $meeting_info['response'] ) );
						...
						// Sync the recordings.
						bp_zoom_meeting_fetch_recordings( $zoom_meeting_id );
						...
						// save the meeting.
						$meeting->save();
					} elseif ( 404 === $meeting_info['code'] && ! empty( $meeting_info['response'] ) && isset( $meeting_info['response']->code ) && 3001 === $meeting_info['response']->code ) {
						bp_zoom_meeting_delete( array( 'parent' => $meeting->meeting_id ) );
