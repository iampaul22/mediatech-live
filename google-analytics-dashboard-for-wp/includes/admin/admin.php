	...
	if ( exactmetrics_is_pro_version() ) {
		...
		if ( ExactMetrics()->license->get_site_license_key() ) {
			if ( ExactMetrics()->license->site_license_expired() ) {
				...
				$message = sprintf( esc_html__( 'Your ExactMetrics license is currently disabled. Please %1$sclick here%2$s to login to your ExactMetrics account and use a different license key or purchase a new one.', 'google-analytics-dashboard-for-wp' ), '<a href="' . exactmetrics_get_url( 'admin-notices', 'disabled-license', "https://www.exactmetrics.com/login/" ) . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>' );
			...
			if ( ExactMetrics()->license->network_license_expired() ) {
				...
				$message = sprintf( esc_html__( 'Your ExactMetrics network license is currently disabled. Please %1$sclick here%2$s to login to your ExactMetrics account and use a different license key.', 'google-analytics-dashboard-for-wp' ), '<a href="' . exactmetrics_get_url( 'admin-notices', 'disabled-network-license', "https://www.exactmetrics.com/login/" ) . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>' );
			} else if ( ExactMetrics()->license->network_license_invalid() ) {
				$message = sprintf( esc_html__( 'Your ExactMetrics network license is invalid. Please %1$sclick here%2$s to login to your ExactMetrics account to confirm your license key.', 'google-analytics-dashboard-for-wp' ), '<a href="' . exactmetrics_get_url( 'admin-notices', 'invalid-network-license', "https://www.exactmetrics.com/login/" ) . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>' );
			}
		}
		if ( ! empty( $message ) ) {
			echo '<div class="error"><p>' . $message . '</p></div>';
