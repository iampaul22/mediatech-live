...
if (!empty($resize_on_element_resize)) {
  ...
  if ($resize_on_element_resize === 'body') {
      $html .= 'var res_element_'.$id.' = jQuery("#'.$id.'");'; 
  } else {
      $html .= 'var res_element_'.$id.' = jQuery("#'.$id.'").contents().find("'.esc_html($resize_on_element_resize).'");';
  }
  $html .= '
     }  catch(e) {
        var res_element_'.$id.' = "";
    ...
    if (res_element_'.$id.'.length === 0) {
        ...
        // if Array.isArray
        res_element_'.$id.'.resize(function(){ ';
