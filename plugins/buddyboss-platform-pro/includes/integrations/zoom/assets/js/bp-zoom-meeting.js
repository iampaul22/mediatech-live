/* jshint browser: true */
/* global bp, bp_zoom_meeting_vars, bp_select2, ZoomMtg */
/* @version 1.0.0 */
window.bp = window.bp || {};
...
( function( exports, $ ) {
	...
	bp.Zoom = {
		...
		toggleRecurrence: function (e) {
			...
			if (target.val() == '-1') {
				...
				if (target.val() == '1') {
					...
					interval_type_label.text(bp_zoom_meeting_vars.strings.day);
				...
				if (target.val() == '2') {
					...
					interval_type_label.text(bp_zoom_meeting_vars.strings.week);
					repeat_interval_html = '';
					for (i = 1; i <= 12; i++) {
						repeat_interval_html += '<option value="' + i + '">' + i + '</option>';
					}
				...
				if (target.val() == '3') {
					...
					interval_type_label.text(bp_zoom_meeting_vars.strings.month);
					repeat_interval_html = '';
					for (i = 1; i <= 3; i++) {
						repeat_interval_html += '<option value="' + i + '">' + i + '</option>';
					}
		...
		joinMeetingInBrowser: function (e) {
			...
			if ( typeof bp_zoom_meeting_vars.scripts === 'undefined' && bp_zoom_meeting_vars.scripts.length == 0 ) {
				return false;
			}
			...
			for(i in bp_zoom_meeting_vars.styles ){
				$('<link/>', {
					rel: 'stylesheet',
					type: 'text/css',
					href: bp_zoom_meeting_vars.styles[i]
				}).appendTo('head');
			}

			for(i in bp_zoom_meeting_vars.scripts){
				var script = document.createElement('script');
				script.type = 'text/javascript';
				script.src = bp_zoom_meeting_vars.scripts[i];
			...
			var loadscripts = setInterval(function(){
				if ( scripts_loaded >= bp_zoom_meeting_vars.scripts.length && typeof ZoomMtg !== 'undefined' ) {
					...
					var meetConfig = {
						...
						userName: bp_zoom_meeting_vars.user.name,
