		...
		*/
		...
		function createMinimizedAiExternalJs($newContent,$backend) {
			...
			if ($minifiedContent === false) {
				...
				if ($this->aiContains($minifiedContent , 'var extendedDebug') && $this->aiContains($minifiedContent , 'additional_js_iframe,document.head.appendChild')) {
		...
		function createMinimizedAiJs($backend) {
			...
			if ($minifiedContent === false) {
				...
				// double check if it is somehow the expected content.
				if ($this->aiContains($minifiedContent , 'aiExtraSpace') && $this->aiContains($minifiedContent , '(aiDisableCheckIframes,200)});var ')) {
				    $type = $this->aiContains($newContent, 'Advanced iframe pro') ? 'pro' : 'free';
