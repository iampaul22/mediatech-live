...
<?php     
     ...
     if ($evanto || $isDemo) {
        printTrueFalse(true,$devOptions, __('Support WP multisite', 'advanced-iframe'), 'multi_domain_enabled', __('This is only supported if you select "Use postMessage for communication" to "yes" or "debug". Please read the documentation at "Use postMessage for communication" how to use this setting!', 'advanced-iframe'), "false", '//www.tinywebgallery.com/blog/advanced-iframe/advanced-iframe-pro-demo/external-workaround-with-post-message',true);
...
if (count($config_files) === 0) {
  ...
  foreach ($config_files as $file) {
    ...
    echo ' &nbsp; <a class="confirmation-file post" href="options-general.php?page=advanced-iframe.php&remove-id='.$rid.'">';
