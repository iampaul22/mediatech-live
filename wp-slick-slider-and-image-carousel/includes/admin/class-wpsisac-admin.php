...
class Wpsisac_Admin {
	...
	function wpsisac_slider_category_manage_columns($theme_columns) {
		$new_columns = array(
			...
			'name' => __('Name'),
			'slider_shortcode' => esc_html__( 'Slider Category Shortcode', 'wp-slick-slider-and-image-carousel' ),
			'slug' => __('Slug'),
			'posts' => __('Posts')
		);
	...
	function wpsisac_slider_category_columns($out, $column_name, $theme_id) {
		...
		switch ($column_name) {
			...
			case 'slider_shortcode':
				echo '[slick-slider category="' . esc_attr( $theme_id ). '"]<br />';
				echo '[slick-carousel-slider category="' . esc_attr( $theme_id ). '"]';
			break;
			default:
			break;
		}
