...
if ( ! class_exists( 'BuddyBoss_Updater_Admin' ) ) :
	...
	class BuddyBoss_Updater_Admin {
		...
		public function setup() {
			...
			add_action(
				'wp_ajax_updater_bb_connect_received_message',
				array(
					$this,
		...
		/**
		 * Initize updater for all active licence products.
		 *
		 * @return void
		 */
		...
		public function get_domain( $domain, $debug = false ) {
			...
			if ( count( $arr ) > 2 ) {
				...
				if ( strpos( join( '.', $arr ), '.' ) === false && in_array(
					$arr[0],
					array(
						'localhost',
						'test',
		...
		private function is_network_activated() {
			...
			if ( is_multisite() ) {
				...
				if ( is_plugin_active_for_network( 'buddyboss-updater/buddyboss-updater.php' ) ) {
		...
		public function admin_menu() {
			$this->page_hook_suffix = add_submenu_page(
				$this->settings_page,
				__( 'BuddyBoss License Keys', 'buddyboss-updater' ),
				__( 'License Keys', 'buddyboss-updater' ),
				$this->capability,
				$this->plugin_slug,
				array(
					$this,
		...
		public function show_partial_activations( $package ) {
			...
			if ( ! empty( $products ) ) {
				...
				if ( ! empty( $activated_products ) ) {
					echo "<br><div class='bb_indirect_license'>You already have a valid license key for " . count( $activated_products ) . ' product(s) of this package:-<br>';
		...
		protected function _get_saved_licenses() {
			if ( ! $this->_saved_licenses ) {
				$this->_saved_licenses = get_option( 'bboss_updater_saved_licenses', array() );

				/**
				 * This is added to give the backward compatibility
				 */
				if ( $this->network_activated ) {
					$site_option = get_site_option( 'bboss_updater_saved_licenses' );

					if ( ! empty( $site_option ) && ! empty( $this->_saved_licenses ) ) {
						$this->_saved_licenses = array_merge( $this->_saved_licenses, $site_option );
					} elseif ( ! empty( $site_option ) ) {
						$this->_saved_licenses = $site_option;
					}
				}
			}
		...
		public function admin_notice() {
			...
			if ( ! empty( $notices ) ) {
				...
				echo "<div class='notice notice-warning is-dismissible'>" . $notice
					 . "<button type='button' class='notice-dismiss' onClick='BBossULDissmissNotice(this);'><span class='screen-reader-text'>" . __( 'Dismiss this notice.', 'buddyboss-updater' ) . '</span></button>'
					 . '</div>';
				?>
			...
			if ( ! empty( $this->packages ) ) {
				...
				if ( empty( $saved_licenses ) ) {
					...
					$settings_link = "<a href='" . $url . "'>" . __( 'activate your product licenses', 'buddyboss-updater' ) . '</a>';
					$notice        = sprintf( __( '<strong>Your BuddyBoss products are almost ready.</strong> To get started, please %s.', 'buddyboss-updater' ), $settings_link );
					...
					foreach ( $this->packages as $package_id => $package ) {
						...
						if ( 'inactive' == $package_status ) {
							...
							// we have some buddyboss packages but no license info in db
							// display admin notice
							$settings_link = "Please click <a href='" . $url . "'>" . __( 'here', 'buddyboss-updater' ) . '</a> and update your license.';
							$notice        = sprintf( __( 'This license for <strong>%1$s</strong> is invalid or incomplete. %2$s', 'buddyboss-updater' ), $package['name'], $settings_link );
		...
		public function updater_bb_connect_received_message() {
			...
			$retval['message'] = __( 'Your account was successfully connected. No new activations have been made. If you are attempting to activate a new license key, please make sure that the product is installed on your site, and that an active license key is available in your BuddyBoss.com account.', 'buddyboss-updater' );
		...
		public function process_bulk( $licenses ) {
			...
			if ( ! empty( $licenses_updated ) ) {
				$retval['message'] = sprintf( __( 'Congratulations! License keys for the following product(s) have been activated: %s', 'buddyboss-updater' ), implode( ', ', $licenses_updated ) );
				...
				// 1. $licenses is empty
				if ( empty( $licenses ) ) {
					$retval['message'] = __( 'The connection was successful, however, no license key was activated. We could not find any active license in your account, for any of the installed BuddyBoss products.', 'buddyboss-updater' );
				}
