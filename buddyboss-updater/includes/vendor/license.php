...
class BBoss_License_Helper {
	...
	public function activate( $key, $email ) {
		...
		if ( is_wp_error( $q_response ) || ( $q_response['response']['code'] != 200 ) ) {
			return array( 'status'  => false,
			              'message' => __( 'We are unable to validate the license key. Service unavailable.', 'buddyboss-updater' )
			);
		}
		...
		return array( 'status'  => false,
		              'message' => __( 'We\'ve checked the license key, but it <strong>doesn\'t appear to be a valid BuddyBoss license.</strong> Please double check the license key and try again.', 'buddyboss-updater' )
		);
	}
