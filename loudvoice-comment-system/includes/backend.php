...
/**
 *  Protect dashboard comment links to prevent other plugins from editing them
 */
function oa_loudvoice_load_dashboardjs_scripts()
{
    ...
    wp_enqueue_script('dashboard_script', plugins_url('../assets/js/dashboard.js', __FILE__), array(
        'jQuery'
    ));
}
...
function oa_loudvoice_admin_menu()
{
    ...
    // More+
    $page = add_submenu_page('oa_loudvoice_settings', 'OneAll LoudVoice Comments Platform ' . __('+More', 'oa_loudvoice'), __('+More', 'oa_loudvoice'), 'manage_options', 'oa_loudvoice_more', 'oa_display_loudvoice_more');
    add_action('admin_print_styles-' . $page, 'oa_loudvoice_admin_css');
...
function oa_loudvoice_admin_check_api_settings()
{
    ...
    // Domain
    $api_domain = $api_subdomain . OA_LOUDVOICE_API_BASE;
...
function oa_loudvoice_settings_validate($settings)
{
    ...
    if ($page == 'settings')
    {
        ...
        $sanitzed_settings['disable_wysiwyg'] = (!empty($sanitzed_settings['disable_wysiwyg']) ? 1 : 0);
        ...
        if (isset($sanitzed_settings['api_connection_handler']) and in_array(strtolower($sanitzed_settings['api_connection_handler']), array(
            'curl',
            'fsockopen'
        )))
        {
...
function oa_loudvoice_display_settings()
{
    ...
    ?>
        <div class="wrap">
            <div id="oa_loudvoice">
                <h2>
                    OneAll LoudVoice <?php echo OA_LOUDVOICE_VERSION; ?>
                </h2>
                <h2 class="nav-tab-wrapper">
                ...
                <form method="post" action="options.php">
                    ...
                    <table class="form-table oa_loudvoice_table">
                        ...
                        <?php
        				    $disable_wysiwyg = (!empty($settings['disable_wysiwyg']) ? 1 : 0);
        				?>
        				<tr class="row_odd">
                            <td class="row_multi" rowspan="2" style="width: 250px">
                                <label for="oa_loudvoice_disable_wysiwyg_0">
        						   <?php _e('Enable the integrated WYSIWYG (what you see is what you get) editor?', 'oa_loudvoice');?>
        					     </label>
                            </td>
                            <td>
                                <input type="radio" id="oa_loudvoice_disable_wysiwyg_0" name="oa_loudvoice_settings[disable_wysiwyg]" value="0" <?php echo ($disable_wysiwyg == 0 ? 'checked="checked"' : ''); ?> />
                                <label for="oa_loudvoice_disable_wysiwyg_0">
                                    <?php _e('Yes, enable the LoudVoice WYSIWYG editor', 'oa_loudvoice');?> <strong>(<?php _e('Recommended', 'oa_loudvoice')?>)</strong>
                                </label><br />
                                <span class="description"><?php _e('Adds a toolbar to easily format comments to the LoudVoice widget.', 'oa_loudvoice');?></span>
                            </td>
                        </tr>
                        <tr class="row_odd">
                            <td class="row_multi">
                                <input type="radio" id="oa_loudvoice_disable_wysiwyg_1" name="oa_loudvoice_settings[disable_wysiwyg]" value="1" <?php echo ($disable_wysiwyg != 0 ? 'checked="checked"' : ''); ?> />
                                <label for="oa_loudvoice_disable_wysiwyg_1">
                                    <?php _e('No, disable the LoudVoice WYSIWYG editor', 'oa_loudvoice');?></strong>
                                </label><br />
                                <span class="description"><?php _e('Even is disabled, users can still format their posts using MarkDown syntax.', 'oa_loudvoice');?></span>
                            </td>
                        </tr>
